...
def read_ppt(request, path):
    ...
    store_path = 'F:\\github\\WBFL\\TianWen\\media\\upload\\'
    file_name = path[path.rfind('\\') + 1:path.find('.')]  # 提取路径中的文件名,不包含后缀
...
def read_doc(request,path):  #将doc文档转换成docx文档并读取,path为文件的路径
        ...
        doc.SaveAs('F:\\github\\WBFL\\TianWen\\media\\upload\\' + file_name + '.docx', 16)
        doc.Close()
        word.Quit()
        read_docx(request,'F:\\github\\WBFL\\TianWen\\media\\upload\\' + file_name + '.docx')
